...
public class ModifyPrefab : IModApi
{
    ...
    public ModifyPrefab() {
       ...
       BlockToolSelection blockToolSelection = BlockToolSelection.Instance;
        ...
        {
            if(action.text == Localization.Get("selectionToolsEditBlocksVolume"))
            {
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));         
            }
            else if (action.text == Localization.Get("selectionToolsCopySleeperVolume"))
            {
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
            else if (action.text == Localization.Get("selectionToolsCopyAirBlocks"))
            {
                action.SetIsVisibleDelegate(new NGuiAction.IsVisibleDelegate(GetDebugAsStatic));
            }
            else if (action.text == Localization.Get("selectionToolsClearSelection"))
            {
                action.SetIsEnabledDelegate((NGuiAction.IsEnabledDelegate)(() => GetDebugAsStatic() && blockToolSelection.SelectionActive));
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
            else if (action.text == Localization.Get("selectionToolsFillSelection"))
            {
                action.SetIsEnabledDelegate((NGuiAction.IsEnabledDelegate)(() => GetDebugAsStatic() && blockToolSelection.SelectionActive));
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
            else if (action.text == Localization.Get("selectionToolsRandomFillSelection"))
            {
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
            else if (action.text == Localization.Get("selectionToolsUndo"))
            {
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
            else if (action.text == Localization.Get("selectionToolsRedo"))
            {
                action.SetIsVisibleDelegate((NGuiAction.IsVisibleDelegate)(() => GetDebugAsStatic()));
            }
        }
